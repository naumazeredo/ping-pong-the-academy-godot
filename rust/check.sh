...
DEFAULT_COMMANDS=("fmt" "clippy" "test")
...
function cmd_fmt() {
    ...
    if [[ $(rustup toolchain list) =~ nightly ]]; then
        run cargo +nightly fmt --manifest-path "$rustCratePath/Cargo.toml" --all -- --check
    else
        log -e "${YELLOW}Warning: nightly toolchain not found; stable rustfmt might not pass CI.${END}"
        run cargo fmt --manifest-path "$rustCratePath/Cargo.toml" --all -- --check
    fi
}

function cmd_clippy() {
    run cargo clippy --manifest-path "$rustCratePath/Cargo.toml" --all-targets "${extraCargoArgs[@]}" -- \
        -D clippy::suspicious \
        -D clippy::style \
        -D clippy::complexity \
        -D clippy::perf \
...
function cmd_klippy() {
    run cargo clippy --manifest-path "$rustCratePath/Cargo.toml" --fix --all-targets "${extraCargoArgs[@]}" -- \
        -D clippy::suspicious \
        -D clippy::style \
        -D clippy::complexity \
        -D clippy::perf \
...
function cmd_test() {
    run cargo test --manifest-path "$rustCratePath/Cargo.toml" "${extraCargoArgs[@]}"
}

function cmd_doc() {
    run cargo doc --lib --manifest-path "$rustCratePath/Cargo.toml" -p godot --no-deps "${extraCargoArgs[@]}"
}

function cmd_dok() {
    run cargo doc --lib --manifest-path "$rustCratePath/Cargo.toml" -p godot --no-deps "${extraCargoArgs[@]}" --open
}
...
while [[ $# -gt 0 ]]; do
    ...
    case "$arg" in
        ...
        --double)
            ...
            ;;
        fmt | test | clippy | klippy | doc | dok)
            cmds+=("$arg")
            ;;
        -a | --api-version)
            if [[ -z "$2" || "$2" == -* ]]; then
